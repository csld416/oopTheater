...
public class LogoPanel extends JPanel {
    ...
    public LogoPanel() {
        setPreferredSize(new Dimension(Const.LOGO_WIDTH, Const.LOGO_HEIGHT));
        setLayout(null); // no layout manager
        setOpaque(false); // transparent background

        JLabel logoLabel = new JLabel();
        logoLabel.setBounds(0, 0, Const.LOGO_WIDTH, Const.LOGO_HEIGHT);

        // Load and scale the logo image
        ImageIcon icon = new ImageIcon(getClass().getResource("/icons/Logo.png"));
        Image scaledImage = icon.getImage().getScaledInstance(Const.LOGO_WIDTH,
                Const.LOGO_HEIGHT,
                Image.SCALE_SMOOTH
        );
